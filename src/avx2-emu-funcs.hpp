...
public:
    ...
    avx2_mask_helper_lut_gen32(){build_lut();}
};
avx2_mask_helper_lut_gen32 avx2_mask_helper_lut32 = avx2_mask_helper_lut_gen32();

struct avx2_mask_helper32{
    __m256i mask;
    
    avx2_mask_helper32(int m){mask = converter(m);}
    avx2_mask_helper32(__m256i m){mask = m;}
...
public:
    ...
    avx2_mask_helper_lut_gen64(){build_lut();}
};
avx2_mask_helper_lut_gen64 avx2_mask_helper_lut64 = avx2_mask_helper_lut_gen64();

struct avx2_mask_helper64{
    __m256i mask;
    
    avx2_mask_helper64(int m){mask = converter(m);}
    avx2_mask_helper64(__m256i m){mask = m;}
    operator __m256i(){return mask;}
    operator int32_t(){return converter(mask);}
    __m256i operator=(int m){mask = converter(m); return mask;}
...
template <typename T>
typename ymm_vector<T>::opmask_t avx2_emu_fpclassify32(typename ymm_vector<T>::ymm_t x, int mask){
    using vtype = ymm_vector<T>;
...
template <typename T>
typename ymm_vector<T>::opmask_t avx2_emu_fpclassify64(typename ymm_vector<T>::ymm_t x, int mask){
    using vtype = ymm_vector<T>;
    T store[vtype::numlanes];
    vtype::storeu(&store[0], x);
...
template <typename T>
int32_t avx2_double_compressstore32(void * left_addr, void * right_addr, typename ymm_vector<T>::opmask_t k, typename ymm_vector<T>::ymm_t reg){
    using vtype = ymm_vector<T>;
...
template <typename T>
int32_t avx2_double_compressstore64(void * left_addr, void * right_addr, typename ymm_vector<T>::opmask_t k, typename ymm_vector<T>::ymm_t reg){
    using vtype = ymm_vector<T>;
