...
X86_SIMD_SORT_FORCEINLINE zmm_t sort_zmm_64bit(zmm_t zmm)
{
...
template <typename vtype, typename zmm_t = typename vtype::zmm_t, typename index_t = zmm_vector<uint64_t>::zmm_t>
X86_SIMD_SORT_FORCEINLINE zmm_t sort_zmm_64bit(zmm_t key_zmm,index_t &index_zmm)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
    key_zmm = cmp_merge<vtype>(
            key_zmm, vtype::template shuffle<SHUFFLE_MASK(1, 1, 1, 1)>(key_zmm), 
            index_zmm, zmm_vector<uint64_t>::template shuffle<SHUFFLE_MASK(1, 1, 1, 1)>(index_zmm),
            0xAA);
    key_zmm = cmp_merge<vtype>(
            key_zmm,vtype::permutexvar(_mm512_set_epi64(NETWORK_64BIT_1), key_zmm),
            index_zmm,zmm_vector<uint64_t>::permutexvar(_mm512_set_epi64(NETWORK_64BIT_1), index_zmm),
            0xCC);
    key_zmm = cmp_merge<vtype>(
            key_zmm, vtype::template shuffle<SHUFFLE_MASK(1, 1, 1, 1)>(key_zmm), 
            index_zmm, zmm_vector<uint64_t>::template shuffle<SHUFFLE_MASK(1, 1, 1, 1)>(index_zmm),
            0xAA);
    key_zmm = cmp_merge<vtype>(
            key_zmm, vtype::permutexvar(rev_index, key_zmm),
            index_zmm,zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm),
            0xF0);
    key_zmm = cmp_merge<vtype>(
            key_zmm,vtype::permutexvar(_mm512_set_epi64(NETWORK_64BIT_3), key_zmm),
            index_zmm,zmm_vector<uint64_t>::permutexvar(_mm512_set_epi64(NETWORK_64BIT_3), index_zmm),
            0xCC);
    key_zmm = cmp_merge<vtype>(
            key_zmm, vtype::template shuffle<SHUFFLE_MASK(1, 1, 1, 1)>(key_zmm),
            index_zmm, zmm_vector<uint64_t>::template shuffle<SHUFFLE_MASK(1, 1, 1, 1)>(index_zmm),
...
template <typename vtype, typename zmm_t = typename vtype::zmm_t>
X86_SIMD_SORT_FORCEINLINE zmm_t bitonic_merge_zmm_64bit(zmm_t zmm)
{
...
// Assumes zmm is bitonic and performs a recursive half cleaner
template <typename vtype, typename zmm_t = typename vtype::zmm_t, typename index_t = zmm_vector<uint64_t>::zmm_t>
X86_SIMD_SORT_FORCEINLINE zmm_t bitonic_merge_zmm_64bit(zmm_t key_zmm,zmm_vector<uint64_t>::zmm_t &index_zmm)
{
    ...
    key_zmm = cmp_merge<vtype>(
            ...
            index_zmm,
             zmm_vector<uint64_t>::permutexvar(_mm512_set_epi64(NETWORK_64BIT_4), index_zmm),
            0xF0);
    // 2) half_cleaner[4]
    key_zmm = cmp_merge<vtype>(
            key_zmm,
            ...
            index_zmm,
             zmm_vector<uint64_t>::permutexvar(_mm512_set_epi64(NETWORK_64BIT_3), index_zmm),
            0xCC);
    // 3) half_cleaner[1]
    key_zmm = cmp_merge<vtype>(
            key_zmm,  vtype::template shuffle<SHUFFLE_MASK(1, 1, 1, 1)>(key_zmm), 
...
template <typename vtype, typename zmm_t = typename vtype::zmm_t>
X86_SIMD_SORT_FORCEINLINE void bitonic_merge_two_zmm_64bit(zmm_t &zmm1,
                                                           zmm_t &zmm2)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
...
// Assumes zmm1 and zmm2 are sorted and performs a recursive half cleaner
template <typename vtype, typename zmm_t = typename vtype::zmm_t, typename index_t = zmm_vector<uint64_t>::zmm_t>
X86_SIMD_SORT_FORCEINLINE void bitonic_merge_two_zmm_64bit(zmm_t &key_zmm1,
                                                           zmm_t &key_zmm2, index_t &index_zmm1,
                                                           index_t &index_zmm2)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
    // 1) First step of a merging network: coex of zmm1 and zmm2 reversed
    ...
    index_t index_zmm3=zmm_vector<uint64_t>::mask_mov(index_zmm2,vtype::eq(key_zmm3,key_zmm1),index_zmm1);
    index_t index_zmm4=zmm_vector<uint64_t>::mask_mov(index_zmm1,vtype::eq(key_zmm3,key_zmm1),index_zmm2);
...
// half cleaner
template <typename vtype, typename zmm_t = typename vtype::zmm_t>
X86_SIMD_SORT_FORCEINLINE void bitonic_merge_four_zmm_64bit(zmm_t *zmm)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
...
template <typename vtype, typename zmm_t = typename vtype::zmm_t, typename index_t = zmm_vector<uint64_t>::zmm_t>
X86_SIMD_SORT_FORCEINLINE void bitonic_merge_four_zmm_64bit(zmm_t *key_zmm,index_t *index_zmm)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
    // 1) First step of a merging network
    zmm_t key_zmm2r = vtype::permutexvar(rev_index, key_zmm[2]);
    zmm_t key_zmm3r = vtype::permutexvar(rev_index, key_zmm[3]);
    index_t index_zmm2r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[2]);
    index_t index_zmm3r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[3]);
    ...
    index_t index_zmm_t1=zmm_vector<uint64_t>::mask_mov(index_zmm3r,vtype::eq(key_zmm_t1,key_zmm[0]),index_zmm[0]);
    index_t index_zmm_m1=zmm_vector<uint64_t>::mask_mov(index_zmm[0],vtype::eq(key_zmm_t1,key_zmm[0]),index_zmm3r);  
    index_t index_zmm_t2=zmm_vector<uint64_t>::mask_mov(index_zmm2r,vtype::eq(key_zmm_t2,key_zmm[1]),index_zmm[1]);
    index_t index_zmm_m2=zmm_vector<uint64_t>::mask_mov(index_zmm[1],vtype::eq(key_zmm_t2,key_zmm[1]),index_zmm2r);   


    // 2) Recursive half clearer: 16
    zmm_t key_zmm_t3 = vtype::permutexvar(rev_index, key_zmm_m2);
    zmm_t key_zmm_t4 = vtype::permutexvar(rev_index, key_zmm_m1);
    index_t index_zmm_t3 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m2);
    index_t index_zmm_t4 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m1);    
    ...
    index_t index_zmm0 = zmm_vector<uint64_t>::mask_mov(index_zmm_t2,vtype::eq(key_zmm0,key_zmm_t1),index_zmm_t1);
    index_t index_zmm1 = zmm_vector<uint64_t>::mask_mov(index_zmm_t1,vtype::eq(key_zmm0,key_zmm_t1),index_zmm_t2);
    index_t index_zmm2 = zmm_vector<uint64_t>::mask_mov(index_zmm_t4,vtype::eq(key_zmm2,key_zmm_t3),index_zmm_t3);
    index_t index_zmm3 = zmm_vector<uint64_t>::mask_mov(index_zmm_t3,vtype::eq(key_zmm2,key_zmm_t3),index_zmm_t4);
...
template <typename vtype, typename zmm_t = typename vtype::zmm_t>
X86_SIMD_SORT_FORCEINLINE void bitonic_merge_eight_zmm_64bit(zmm_t *zmm)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
...
template <typename vtype, typename zmm_t = typename vtype::zmm_t, typename index_t = zmm_vector<uint64_t>::zmm_t>
X86_SIMD_SORT_FORCEINLINE void bitonic_merge_eight_zmm_64bit(zmm_t *key_zmm,index_t *index_zmm)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
    ...
    index_t index_zmm4r =  zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[4]);
    index_t index_zmm5r =  zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[5]);
    index_t index_zmm6r =  zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[6]);
    index_t index_zmm7r =  zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[7]);
    ...
    index_t index_zmm_t1= zmm_vector<uint64_t>::mask_mov(index_zmm7r, vtype::eq(key_zmm_t1,key_zmm[0]),index_zmm[0]);
    index_t index_zmm_m1= zmm_vector<uint64_t>::mask_mov(index_zmm[0], vtype::eq(key_zmm_t1,key_zmm[0]),index_zmm7r);  
    index_t index_zmm_t2= zmm_vector<uint64_t>::mask_mov(index_zmm6r, vtype::eq(key_zmm_t2,key_zmm[1]),index_zmm[1]);
    index_t index_zmm_m2= zmm_vector<uint64_t>::mask_mov(index_zmm[1], vtype::eq(key_zmm_t2,key_zmm[1]),index_zmm6r);   
    index_t index_zmm_t3= zmm_vector<uint64_t>::mask_mov(index_zmm5r, vtype::eq(key_zmm_t3,key_zmm[2]),index_zmm[2]);
    index_t index_zmm_m3= zmm_vector<uint64_t>::mask_mov(index_zmm[2], vtype::eq(key_zmm_t3,key_zmm[2]),index_zmm5r);  
    index_t index_zmm_t4= zmm_vector<uint64_t>::mask_mov(index_zmm4r, vtype::eq(key_zmm_t4,key_zmm[3]),index_zmm[3]);
    index_t index_zmm_m4= zmm_vector<uint64_t>::mask_mov(index_zmm[3], vtype::eq(key_zmm_t4,key_zmm[3]),index_zmm4r);   
    ...
    index_t index_zmm_t5 =  zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m4);
    index_t index_zmm_t6 =  zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m3);    
    index_t index_zmm_t7 =  zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m2);
    index_t index_zmm_t8 =  zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m1);    
...
template <typename vtype, typename zmm_t = typename vtype::zmm_t>
X86_SIMD_SORT_FORCEINLINE void bitonic_merge_sixteen_zmm_64bit(zmm_t *zmm)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
...
template <typename vtype, typename zmm_t = typename vtype::zmm_t, typename index_t = zmm_vector<uint64_t>::zmm_t>
X86_SIMD_SORT_FORCEINLINE void bitonic_merge_sixteen_zmm_64bit(zmm_t *key_zmm,index_t *index_zmm)
{
    const __m512i rev_index = _mm512_set_epi64(NETWORK_64BIT_2);
    ...
    index_t index_zmm8r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[8]);
    index_t index_zmm9r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[9]);
    index_t index_zmm10r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[10]);
    index_t index_zmm11r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[11]);
    index_t index_zmm12r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[12]);
    index_t index_zmm13r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[13]);
    index_t index_zmm14r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[14]);
    index_t index_zmm15r = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm[15]);
    ...
    index_t index_zmm_t1=zmm_vector<uint64_t>::mask_mov(index_zmm15r,vtype::eq(key_zmm_t1,key_zmm[0]),index_zmm[0]);
    index_t index_zmm_m1=zmm_vector<uint64_t>::mask_mov(index_zmm[0],vtype::eq(key_zmm_t1,key_zmm[0]),index_zmm15r);  
    index_t index_zmm_t2=zmm_vector<uint64_t>::mask_mov(index_zmm14r,vtype::eq(key_zmm_t2,key_zmm[1]),index_zmm[1]);
    index_t index_zmm_m2=zmm_vector<uint64_t>::mask_mov(index_zmm[1],vtype::eq(key_zmm_t2,key_zmm[1]),index_zmm14r);   
    index_t index_zmm_t3=zmm_vector<uint64_t>::mask_mov(index_zmm13r,vtype::eq(key_zmm_t3,key_zmm[2]),index_zmm[2]);
    index_t index_zmm_m3=zmm_vector<uint64_t>::mask_mov(index_zmm[2],vtype::eq(key_zmm_t3,key_zmm[2]),index_zmm13r);  
    index_t index_zmm_t4=zmm_vector<uint64_t>::mask_mov(index_zmm12r,vtype::eq(key_zmm_t4,key_zmm[3]),index_zmm[3]);
    index_t index_zmm_m4=zmm_vector<uint64_t>::mask_mov(index_zmm[3],vtype::eq(key_zmm_t4,key_zmm[3]),index_zmm12r);   
    
    index_t index_zmm_t5=zmm_vector<uint64_t>::mask_mov(index_zmm11r,vtype::eq(key_zmm_t5,key_zmm[4]),index_zmm[4]);
    index_t index_zmm_m5=zmm_vector<uint64_t>::mask_mov(index_zmm[4],vtype::eq(key_zmm_t5,key_zmm[4]),index_zmm11r);  
    index_t index_zmm_t6=zmm_vector<uint64_t>::mask_mov(index_zmm10r,vtype::eq(key_zmm_t6,key_zmm[5]),index_zmm[5]);
    index_t index_zmm_m6=zmm_vector<uint64_t>::mask_mov(index_zmm[5],vtype::eq(key_zmm_t6,key_zmm[5]),index_zmm10r);   
    index_t index_zmm_t7=zmm_vector<uint64_t>::mask_mov(index_zmm9r,vtype::eq(key_zmm_t7,key_zmm[6]),index_zmm[6]);
    index_t index_zmm_m7=zmm_vector<uint64_t>::mask_mov(index_zmm[6],vtype::eq(key_zmm_t7,key_zmm[6]),index_zmm9r);  
    index_t index_zmm_t8=zmm_vector<uint64_t>::mask_mov(index_zmm8r,vtype::eq(key_zmm_t8,key_zmm[7]),index_zmm[7]);
    index_t index_zmm_m8=zmm_vector<uint64_t>::mask_mov(index_zmm[7],vtype::eq(key_zmm_t8,key_zmm[7]),index_zmm8r);   
    ...
    index_t index_zmm_t9 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m8);
    index_t index_zmm_t10 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m7);    
    index_t index_zmm_t11 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m6);
    index_t index_zmm_t12 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m5);
    index_t index_zmm_t13 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m4);
    index_t index_zmm_t14 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m3);    
    index_t index_zmm_t15 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m2);
    index_t index_zmm_t16 = zmm_vector<uint64_t>::permutexvar(rev_index, index_zmm_m1);    
...
X86_SIMD_SORT_FORCEINLINE void sort_8_64bit(type_t *keys,uint64_t *indexes, int32_t N)
{
    typename vtype::opmask_t load_mask = (0x01 << N) - 0x01;
    typename vtype::zmm_t key_zmm
            = vtype::mask_loadu(vtype::zmm_max(), load_mask, keys);
    if(indexes){
        zmm_vector<uint64_t>::zmm_t index_zmm
            = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask, indexes);
        vtype::mask_storeu(keys, load_mask, sort_zmm_64bit<vtype>(key_zmm,index_zmm));
        zmm_vector<uint64_t>::mask_storeu(indexes, load_mask,index_zmm);
    }else{
        vtype::mask_storeu(keys, load_mask, sort_zmm_64bit<vtype>(key_zmm));
    } 
    
}

template <typename vtype, typename type_t>
X86_SIMD_SORT_FORCEINLINE void sort_16_64bit(type_t *keys,uint64_t *indexes, int32_t N)
{
    if (N <= 8) {
    ...
    using index_t = zmm_vector<uint64_t>::zmm_t;
    ...
    if(indexes){
        index_t index_zmm1 = zmm_vector<uint64_t>::loadu(indexes);
        index_t index_zmm2 = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask, indexes + 8);
        key_zmm1 = sort_zmm_64bit<vtype>(key_zmm1,index_zmm1);
        key_zmm2 = sort_zmm_64bit<vtype>(key_zmm2,index_zmm2);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm1,key_zmm2,index_zmm1,index_zmm2);
        zmm_vector<uint64_t>::storeu(indexes, index_zmm1);
        zmm_vector<uint64_t>::mask_storeu(indexes + 8, load_mask, index_zmm2);
    }else{
        key_zmm1 = sort_zmm_64bit<vtype>(key_zmm1);
        key_zmm2 = sort_zmm_64bit<vtype>(key_zmm2);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm1,key_zmm2);      
    }
...
template <typename vtype, typename type_t>
X86_SIMD_SORT_FORCEINLINE void sort_32_64bit(type_t *keys,uint64_t *indexes, int32_t N)
{
    if (N <= 16) {
    ...
    if(indexes){
        ...
        key_zmm[0] = sort_zmm_64bit<vtype>(key_zmm[0]);
        key_zmm[1] = sort_zmm_64bit<vtype>(key_zmm[1]);
    }
    ...
    if(indexes){
        index_zmm[2] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask1, indexes + 16);
        index_zmm[3] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask2, indexes + 24);
        key_zmm[2] = sort_zmm_64bit<vtype>(key_zmm[2],index_zmm[2]);
        key_zmm[3] = sort_zmm_64bit<vtype>(key_zmm[3],index_zmm[3]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[0], key_zmm[1],index_zmm[0], index_zmm[1]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[2], key_zmm[3],index_zmm[2], index_zmm[3]);
        ...
        key_zmm[2] = sort_zmm_64bit<vtype>(key_zmm[2]);
        key_zmm[3] = sort_zmm_64bit<vtype>(key_zmm[3]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[0], key_zmm[1]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[2], key_zmm[3]);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm);   
    }
...
template <typename vtype, typename type_t>
X86_SIMD_SORT_FORCEINLINE void sort_64_64bit(type_t *keys, uint64_t *indexes, int32_t N)
{
    if (N <= 32) {
    ...
    if(indexes){
        index_zmm[4] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask1, indexes + 32);
        index_zmm[5] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask2, indexes + 40);
        index_zmm[6] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask3, indexes + 48);
        index_zmm[7] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask4, indexes + 56);
        ...
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[0], key_zmm[1],index_zmm[0], index_zmm[1]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[2], key_zmm[3],index_zmm[2], index_zmm[3]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[4], key_zmm[5],index_zmm[4], index_zmm[5]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[6], key_zmm[7],index_zmm[6], index_zmm[7]);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm,index_zmm);
        ...
        key_zmm[4] = sort_zmm_64bit<vtype>(key_zmm[4]);
        key_zmm[5] = sort_zmm_64bit<vtype>(key_zmm[5]);
        key_zmm[6] = sort_zmm_64bit<vtype>(key_zmm[6]);
        key_zmm[7] = sort_zmm_64bit<vtype>(key_zmm[7]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[0], key_zmm[1]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[2], key_zmm[3]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[4], key_zmm[5]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[6], key_zmm[7]);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm + 4);
        bitonic_merge_eight_zmm_64bit<vtype>(key_zmm);      
    }
...
template <typename vtype, typename type_t>
X86_SIMD_SORT_FORCEINLINE void sort_128_64bit(type_t *keys, uint64_t *indexes, int32_t N)
{
    if (N <= 64) {
    ...
    if(indexes){
        index_zmm[8] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask1, indexes + 64);
        index_zmm[9] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask2, indexes + 72);
        index_zmm[10] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask3, indexes + 80);
        index_zmm[11] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask4, indexes + 88);
        index_zmm[12] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask5, indexes + 96);
        index_zmm[13] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask6, indexes + 104);
        index_zmm[14] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask7, indexes + 112);
        index_zmm[15] = zmm_vector<uint64_t>::mask_loadu(zmm_vector<uint64_t>::zmm_max(), load_mask8, indexes + 120);
        ...
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[4], key_zmm[5],index_zmm[4], index_zmm[5]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[6], key_zmm[7],index_zmm[6], index_zmm[7]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[8], key_zmm[9],index_zmm[8], index_zmm[9]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[10], key_zmm[11],index_zmm[10], index_zmm[11]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[12], key_zmm[13],index_zmm[12], index_zmm[13]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[14], key_zmm[15],index_zmm[14], index_zmm[15]);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm,index_zmm);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm + 4,index_zmm + 4);
        ...
        key_zmm[8] = sort_zmm_64bit<vtype>(key_zmm[8]);
        key_zmm[9] = sort_zmm_64bit<vtype>(key_zmm[9]);
        key_zmm[10] = sort_zmm_64bit<vtype>(key_zmm[10]);
        key_zmm[11] = sort_zmm_64bit<vtype>(key_zmm[11]);
        key_zmm[12] = sort_zmm_64bit<vtype>(key_zmm[12]);
        key_zmm[13] = sort_zmm_64bit<vtype>(key_zmm[13]);
        key_zmm[14] = sort_zmm_64bit<vtype>(key_zmm[14]);
        key_zmm[15] = sort_zmm_64bit<vtype>(key_zmm[15]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[0], key_zmm[1]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[2], key_zmm[3]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[4], key_zmm[5]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[6], key_zmm[7]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[8], key_zmm[9]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[10], key_zmm[11]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[12], key_zmm[13]);
        bitonic_merge_two_zmm_64bit<vtype>(key_zmm[14], key_zmm[15]);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm + 4);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm + 8);
        bitonic_merge_four_zmm_64bit<vtype>(key_zmm + 12);
        bitonic_merge_eight_zmm_64bit<vtype>(key_zmm);
        bitonic_merge_eight_zmm_64bit<vtype>(key_zmm + 8);
        bitonic_merge_sixteen_zmm_64bit<vtype>(key_zmm);
    }
    vtype::storeu(keys, key_zmm[0]);
    vtype::storeu(keys + 8, key_zmm[1]);
    vtype::storeu(keys + 16, key_zmm[2]);
...
template <typename vtype, typename type_t>
X86_SIMD_SORT_FORCEINLINE type_t get_pivot_64bit(type_t *keys, uint64_t *indexes,
                                                 const int64_t left,
                                                 const int64_t right)
{
    // median of 8
    ...
    if(indexes) 
    {
      index_vec=zmm_vector<uint64_t>::template i64gather<sizeof(uint64_t)>(rand_index, indexes);
      sort = sort_zmm_64bit<vtype>(key_vec,index_vec);
    }else{
      //index_vec=vtype::template i64gather<sizeof(type_t)>(rand_index, indexes);
      sort = sort_zmm_64bit<vtype>(key_vec);
    }
...
template <typename vtype, typename type_t>
inline void 
heapify(type_t* keys, uint64_t* indexes, int64_t idx, int64_t size)
{
  int64_t i = idx;
  while(true) {
    int64_t j = 2 * i + 1;
...
template <typename vtype, typename type_t>
inline void 
heap_sort(type_t* keys, uint64_t* indexes, int64_t size)
{
...
template <typename vtype, typename type_t>
inline void
qsort_64bit_(type_t *keys,uint64_t *indexes,  int64_t left, int64_t right, int64_t max_iters)
{
    /*
     * Resort to std::sort if quicksort isnt making any progress
     */
    if (max_iters <= 0) {
        if(indexes)heap_sort<vtype>(keys + left, indexes + left, right - left + 1);
        else  std::sort(keys + left, keys + right + 1);
        return;
    }
    /*
     ...
     */
    if (right + 1 - left <= 128) {
        if(indexes) sort_128_64bit<vtype>(keys + left, indexes + left, (int32_t)(right + 1 - left));
        else       sort_128_64bit<vtype>(keys + left, (uint64_t*)NULL, (int32_t)(right + 1 - left));
        return;
    }
    ...
    if (pivot != smallest)
        qsort_64bit_<vtype>(keys,indexes, left, pivot_index - 1, max_iters - 1);
    if (pivot != biggest)
        qsort_64bit_<vtype>(keys,indexes, pivot_index, right, max_iters - 1);
}

X86_SIMD_SORT_FORCEINLINE int64_t replace_nan_with_inf(double *arr,
                                                       int64_t arrsize)
{
    int64_t nan_count = 0;
...
template <>
inline void avx512_qsort<int64_t>(int64_t *keys, uint64_t *indexes, int64_t arrsize)
{
    if (arrsize > 1) {
        qsort_64bit_<zmm_vector<int64_t>, int64_t>(
                keys,indexes, 0, arrsize - 1, 2 * (63 - __builtin_clzll(arrsize)));
    }
}

template <>
inline void avx512_qsort<uint64_t>(uint64_t *keys,uint64_t *indexes, int64_t arrsize)
{
    if (arrsize > 1) {
...
template <>
inline void avx512_qsort<double>(double *keys,uint64_t *indexes, int64_t arrsize)
{
    if (arrsize > 1) {
