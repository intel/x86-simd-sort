...
#define YMM_MAX_DOUBLE _mm256_set1_pd(X86_SIMD_SORT_INFINITY)
#define YMM_MAX_UINT64 _mm256_set1_epi64(X86_SIMD_SORT_MAX_UINT64)
#define YMM_MAX_INT64 _mm256_set1_epi64(X86_SIMD_SORT_MAX_INT64)
#define YMM_MAX_FLOAT _mm256_set1_ps(X86_SIMD_SORT_INFINITYF)
#define YMM_MAX_UINT _mm256_set1_epi32(X86_SIMD_SORT_MAX_UINT32)
#define YMM_MAX_INT _mm256_set1_epi32(X86_SIMD_SORT_MAX_INT32)
#define YMM_MAX_HALF _mm256_set1_epi16(X86_SIMD_SORT_INFINITYH)
#define YMM_MAX_UINT16 _mm256_set1_epi16(X86_SIMD_SORT_MAX_UINT16)
#define YMM_MAX_INT16 _mm256_set1_epi16(X86_SIMD_SORT_MAX_INT16)
...
template <typename vtype,
          typename ymm_t = typename vtype::ymm_t,
          typename opmask_t = typename vtype::opmask_t>
static inline ymm_t cmp_merge(ymm_t in1, ymm_t in2, opmask_t mask)
{
    ymm_t min = vtype::min(in2, in1);
    ymm_t max = vtype::max(in2, in1);
                                     ...
                                     type_t *biggest)
{
    ...
    ymm_t pivot_vec = vtype::set1(pivot);
    ymm_t min_vec = vtype::set1(*smallest);
    ymm_t max_vec = vtype::set1(*biggest);

    if (right - left == vtype::numlanes) {
        ymm_t vec = vtype::loadu(arr + left);
    ...
    ymm_t vec_left = vtype::loadu(arr + left);
    ymm_t vec_right = vtype::loadu(arr + (right - vtype::numlanes));
                                              ...
                                              type_t *biggest)
{
    ...
    ymm_t vec_left[num_unroll], vec_right[num_unroll];
#pragma GCC unroll 8
...
static void COEX(ymm_t1 &key1, ymm_t1 &key2, ymm_t2 &index1, ymm_t2 &index2)
{
    ymm_t1 key_t1 = vtype1::min(key1, key2);
    ymm_t1 key_t2 = vtype1::max(key1, key2);
...
static inline ymm_t1 cmp_merge(ymm_t1 in1,
                               ymm_t1 in2,
                               ymm_t2 &indexes1,
                               ymm_t2 indexes2,
                               opmask_t mask)
{
    ymm_t1 tmp_keys = cmp_merge<vtype1>(in1, in2, mask);
    indexes1 = vtype2::mask_mov(indexes2, vtype1::eq(tmp_keys, in1), indexes1);
    return tmp_keys; // 0 -> min, 1 -> max
}

/*
 * Parition one YMM register based on the pivot and returns the index of the
...
static inline int32_t partition_vec(type_t1 *keys,
                                    ...
                                    int64_t left,
                                    int64_t right,
                                    const ymm_t1 keys_vec,
                                    const ymm_t2 indexes_vec,
                                    const ymm_t1 pivot_vec,
                                    ymm_t1 *smallest_vec,
                                    ymm_t1 *biggest_vec)
{
                                     ...
                                     type_t1 *biggest)
{
    ...
    ymm_t1 pivot_vec = vtype1::set1(pivot);
    ymm_t1 min_vec = vtype1::set1(*smallest);
    ymm_t1 max_vec = vtype1::set1(*biggest);

    if (right - left == vtype1::numlanes) {
        ymm_t1 keys_vec = vtype1::loadu(keys + left);
        int32_t amount_gt_pivot;

        ymm_t2 indexes_vec = vtype2::loadu(indexes + left);
    ...
    ymm_t1 keys_vec_left = vtype1::loadu(keys + left);
    ymm_t1 keys_vec_right = vtype1::loadu(keys + (right - vtype1::numlanes));
    ymm_t2 indexes_vec_left;
    ymm_t2 indexes_vec_right;
