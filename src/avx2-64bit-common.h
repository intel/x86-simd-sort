...
X86_SIMD_SORT_INLINE ymm_t bitonic_merge_ymm_64bit(ymm_t ymm)
{
    ...
    ymm = cmp_merge<vtype>(
            ymm,
            vtype::template permutexvar<SHUFFLE_MASK(1, 0, 3, 2)>(ymm),
            oxCC);
    ymm = cmp_merge<vtype>(
            ymm,
...
X86_SIMD_SORT_INLINE int64_t replace_nan_with_inf(double *arr, int64_t arrsize)
{
    ...
    while (arrsize > 0) {
        ...
        __m256i nanmask = _mm256_castpd_si256(
                _mm256_cmp_pd(in_ymm, in_ymm, _CMP_NEQ_UQ));
                                            ...
                                            const int64_t right)
{
    ...
    __m256i rand_index = _mm256_set_epi64x(
